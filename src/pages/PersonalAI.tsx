        ...
        {voiceActive && (
          <div className="bg-slate-800 rounded-xl shadow-lg border border-emerald-500/30 flex flex-col p-6 animate-in fade-in slide-in-from-top-4">
            <div className="flex justify-between items-center mb-4">
              ...
              <span className="text-xs text-emerald-400/70 border border-emerald-500/30 px-2 py-1 rounded-full">Listening & Speaking...</span>
            </div>
            <div className="bg-slate-900/50 rounded-lg p-8 flex flex-col items-center justify-center min-h-[120px]">
              <div className="flex items-end justify-center gap-2 h-16 w-full max-w-[200px]">
                {voiceVolume.map((vol, i) => (
                  <div 
                    key={i} 
                    className="w-4 bg-emerald-500 rounded-t-sm transition-all duration-150 ease-out" 
                    style={{ height: `${vol}%` }}
                  ></div>
                ))}
              </div>
